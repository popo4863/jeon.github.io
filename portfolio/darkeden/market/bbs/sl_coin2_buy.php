...
$charcheck2 = mysql_query("SELECT * FROM `Player` WHERE `PlayerID` = '$user_id' AND `point` <= '5000'");
...
list($logged) = mysql_fetch_row(mysql_query("SELECT `LogOn` FROM `Player` WHERE `PlayerID` = '$user_id'"));
	...
	else
	{
		...
		$query1 = mysql_query("SELECT * FROM `Player` WHERE `PlayerID` = '$user_id' AND `point`-'5000'");
		while ($row = mysql_fetch_assoc($query1))
		{
			if($query['Type'] == 1)
			{
				mysql_query("INSERT INTO `GoodsListObject` (`BuyID`, `World`, `PlayerID`, `Name`, `GoodsID`, `Num`) VALUES ('1','1','$user_id','$char','4','1')");
				mysql_query("UPDATE `Player` SET `point` = `point`-'5000' WHERE `PlayerID` = '$user_id'") or exit("등록시 오류가 발생했습니다. <br><br>".mysql_error());
			}
			mysql_query("INSERT INTO `GoodsListObject` (`BuyID`, `World`, `PlayerID`, `Name`, `GoodsID`, `Num`) VALUES ('1','1','$user_id','$char','4','1')");
			mysql_query("UPDATE `Player` SET `point` = `point`-'5000' WHERE `PlayerID` = '$user_id'") or exit("등록시 오류가 발생했습니다. <br><br>".mysql_error());
		}
